import { useStores as useStores$1, useCollection as useCollection$1, useApi, getFieldsFromTemplate as getFieldsFromTemplate$1, defineInterface } from '@directus/extensions-sdk';
import { useI18n } from 'vue-i18n';
import { computed, ref, watch, shallowRef, inject, defineComponent, reactive, toRefs, onMounted, isRef, onUnmounted, resolveComponent, resolveDirective, openBlock, createBlock, withCtx, withDirectives, createElementBlock, normalizeClass, createVNode, createElementVNode, withModifiers, createCommentVNode, vShow, resolveDynamicComponent, normalizeProps, guardReactiveProps, mergeProps, createSlots, renderSlot, Fragment, createTextVNode, toDisplayString, renderList, pushScopeId, popScopeId } from 'vue';
...
function useActions(value, relation, emit) {
    // Returns the junction item with the given Id.
    function getJunctionItem(id) {
        const { junctionPkField } = relation.value;
        if (value.value === null)
            return null;
        return (value.value.find((item) => lodash.exports.get(item, junctionPkField) === id || (item === id)) || null);
    }
    ...
    function getNewSelectedItems() {
        ...
        if (value.value === null || junctionField === null)
            return [];
        return value.value.filter((item) => typeof item === 'object' && junctionField in item && typeof item[junctionField] !== 'object');
    }
    // Returns all items that do not have an existing junction and related item.
    function getNewItems() {
        const { junctionPkField } = relation.value;
        if (value.value === null || junctionPkField === null)
            return [];
        return value.value.filter((item) => typeof item === 'object' && !(item === null || item === void 0 ? void 0 : item[junctionPkField]));
    }
    ...
    function getPrimaryKeys() {
        ...
        return value.value.reduce((acc, item) => {
            const deepId = lodash.exports.get(item, [junctionPkField]);
            if (['string', 'number'].includes(typeof item))
                acc.push(item);
            else if (deepId !== undefined)
                acc.push(deepId);
            return acc;
        }, []);
    }
    ...
    function getRelatedPrimaryKeys() {
        ...
        return value.value.reduce((acc, item) => {
            const relatedId = lodash.exports.get(item, junctionField);
            const deepRelatedId = lodash.exports.get(item, [junctionField, relationPkField]);
            if (relatedId !== undefined)
                acc.push(relatedId);
            else if (deepRelatedId !== undefined)
                acc.push(deepRelatedId);
            return acc;
        }, []);
    }
    function getJunctionFromRelatedId(id, items) {
        const { relationPkField, junctionField } = relation.value;
        return items.find((item) => lodash.exports.get(item, [junctionField, relationPkField]) === id) || null;
    }
    function deleteItem(deletingItem) {
        if (value.value === null)
            return;
        const { junctionField, relationPkField, junctionPkField } = relation.value;
        const junctionId = lodash.exports.get(deletingItem, junctionPkField);
        const relatedId = lodash.exports.get(deletingItem, [junctionField, relationPkField]);
        const newValue = value.value.filter((item) => {
            if (junctionId !== undefined) {
                if (typeof item === 'object') {
                    return lodash.exports.get(item, [junctionPkField]) !== junctionId;
                }
                else {
            ...
            if (relatedId !== undefined) {
                const itemRelatedId = lodash.exports.get(item, [junctionField, relationPkField]);
                if (['string', 'number'].includes(typeof itemRelatedId)) {
                    return itemRelatedId !== relatedId;
                }
                const junctionFieldId = lodash.exports.get(item, [junctionField]);
                if (['string', 'number'].includes(typeof junctionFieldId)) {
                    return junctionFieldId !== relatedId;
                }
            }
            return lodash.exports.isEqual(item, deletingItem) === false;
        });
...
function useRelation(collection, field) {
    const { useRelationsStore, useCollectionsStore } = useStores$1();
    ...
    const junction = computed(() => {
        return relations.value.find((relation) => { var _a; return relation.related_collection === collection.value && ((_a = relation.meta) === null || _a === void 0 ? void 0 : _a.one_field) === field.value; });
    });
    const relation = computed(() => {
        return relations.value.find((relation) => { var _a; return relation.collection === junction.value.collection && relation.field === ((_a = junction.value.meta) === null || _a === void 0 ? void 0 : _a.junction_field); });
    });
    ...
    const { primaryKeyField: junctionPrimaryKeyField, fields: junctionFields } = useCollection$1(junctionCollection.value.collection);
    const { primaryKeyField: relationPrimaryKeyField, fields: relationFields } = useCollection$1(relationCollection.value.collection);
    const relationInfo = computed(() => {
        var _a, _b, _c, _d;
        return {
            junctionPkField: (_a = junctionPrimaryKeyField.value) === null || _a === void 0 ? void 0 : _a.field,
            relationPkField: (_b = relationPrimaryKeyField.value) === null || _b === void 0 ? void 0 : _b.field,
            junctionField: (_c = junction.value.meta) === null || _c === void 0 ? void 0 : _c.junction_field,
            sortField: (_d = junction.value.meta) === null || _d === void 0 ? void 0 : _d.sort_field,
...
function addRelatedPrimaryKeyToFields(currentCollection, fields) {
    ...
    for (const fieldName of fields) {
        ...
        const primaryKeyField = fieldsStore.getPrimaryKeyFieldForCollection((_a = field === null || field === void 0 ? void 0 : field.collection) !== null && _a !== void 0 ? _a : '');
...
function usePreview(value, fields, relation, getNewSelectedItems, getUpdatedItems, getNewItems, getPrimaryKeys) {
    ...
    watch(() => value.value, async (newVal, oldVal) => {
        ...
        const relatedPrimaryKeys = junctionItems.reduce((acc, junction) => {
            const id = lodash.exports.get(junction, junctionField);
            if (id !== null)
                acc.push(id);
            return acc;
        }, []);
        const filteredFields = [...(fields.value.length > 0 ? getRelatedFields(fields.value) : getDefaultFields())];
        if (filteredFields.includes(relationPkField) === false)
            filteredFields.push(relationPkField);
        try {
            let responseData = [];
            ...
            responseData = responseData.map((data) => {
                const id = lodash.exports.get(data, relationPkField);
                const junction = junctionItems.find((junction) => junction[junctionField] === id);
                if (junction === undefined || id === undefined)
                    return;
                const newJunction = lodash.exports.cloneDeep(junction);
            ...
            responseData = responseData
                .map((item) => {
                const updatedItem = updatedItems.find((updated) => lodash.exports.get(updated, [junctionField, relationPkField]) === lodash.exports.get(item, [junctionField, relationPkField]));
                if (updatedItem !== undefined)
                    return lodash.exports.merge(item, updatedItem);
                return item;
            })
                .concat(...newItems);
            if (!initialItems.value.length)
                initialItems.value = responseData;
    ...
    watch(() => fields.value, () => {
        ...
        tableHeaders.value = (fields.value.length > 0 ? fields.value : getDefaultFields().map((field) => `${junctionField}.${field}`))
            .map((fieldKey) => {
            var _a, _b, _c, _d;
            ...
            const header = {
                ...
                width: null,
                field: {
                    display: (_a = field.meta) === null || _a === void 0 ? void 0 : _a.display,
                    displayOptions: (_b = field.meta) === null || _b === void 0 ? void 0 : _b.display_options,
                    interface: (_c = field.meta) === null || _c === void 0 ? void 0 : _c.interface,
                    interfaceOptions: (_d = field.meta) === null || _d === void 0 ? void 0 : _d.options,
                    type: field.type,
                    field: field.field,
                },
            };
    ...
    function getRelatedFields(fields) {
        ...
        return fields.reduce((acc, field) => {
            ...
            if (junctionField === sections[0] && sections.length >= 2)
                acc.push(sections.slice(1).join('.'));
            return acc;
        }, []);
    }
    ...
    async function loadRelatedIds() {
        ...
        try {
            ...
            if (primaryKeys.length > 0) {
                ...
                if (filteredFields.includes(junctionPkField) === false)
                    filteredFields.push(junctionPkField);
                if (filteredFields.includes(junctionField) === false)
                    filteredFields.push(junctionField);
...
function useEdit(value, relation, emit) {
    ...
    function editItem(item) {
        ...
        currentlyEditing.value = lodash.exports.get(item, [junctionPkField], null);
        relatedPrimaryKey.value = lodash.exports.get(item, [junctionField, relationPkField], null);
    }
    function createNew() {
    ...
    function stageEdits(edits) {
        ...
        const newValue = (value.value || []).map((item) => {
            ...
            if (relatedPrimaryKey.value != null) {
                ...
                if (lodash.exports.get(item, [junctionField], null) === id)
                    return edits;
                if (lodash.exports.get(item, [junctionField, relationPkField], null) === id)
                    return edits;
            }
            if (lodash.exports.isEqual(editsAtStart.value, item)) {
                return edits;
            }
            return item;
        });
...
function useSelection(items, initialItems, relation, emit) {
    ...
    const selectedPrimaryKeys = computed(() => {
        ...
        const selectedKeys = items.value.reduce((acc, current) => {
            const key = lodash.exports.get(current, [junctionField, relationPkField]);
            if (key !== undefined)
                acc.push(key);
            return acc;
        }, []);
    ...
    function stageSelection(newSelection) {
        ...
        const selection = newSelection.map((item) => {
            ...
            return {
                ...
                ...draft,
                [junctionField]: {
                    ...initial === null || initial === void 0 ? void 0 : initial[relationPkField],
                    ...draft === null || draft === void 0 ? void 0 : draft[relationPkField],
...
function usePermissions$1(junctionCollection, relationCollection) {
    const { usePermissionsStore, useUserStore } = useStores$1();
    const permissionsStore = usePermissionsStore();
    const userStore = useUserStore();
    const createAllowed = computed(() => {
        var _a;
        const admin = ((_a = userStore.currentUser) === null || _a === void 0 ? void 0 : _a.role.admin_access) === true;
        if (admin)
            return true;
        const hasJunctionPermissions = !!permissionsStore.permissions.find((permission) => permission.action === 'create' && permission.collection === junctionCollection.value.collection);
        const hasRelatedPermissions = !!permissionsStore.permissions.find((permission) => permission.action === 'create' && permission.collection === relationCollection.value.collection);
...
function adjustFieldsForDisplays(fields, parentCollection) {
    const { useFieldsStore } = useStores$1();
    const fieldsStore = useFieldsStore();
    const adjustedFields = fields
        ...
        if (!field)
            return fieldKey;
        if (((_a = field.meta) === null || _a === void 0 ? void 0 : _a.display) === null)
            return fieldKey;
        const display = getDisplay((_b = field.meta) === null || _b === void 0 ? void 0 : _b.display);
        if (!display)
            return fieldKey;
        if (!(display === null || display === void 0 ? void 0 : display.fields))
            return fieldKey;
        let fieldKeys = null;
        ...
        if (typeof display.fields === 'function') {
            fieldKeys = display
                .fields((_c = field.meta) === null || _c === void 0 ? void 0 : _c.display_options, {
...
function useDirectusToken(directusApi) {
    return {
        addQueryToPath,
        getToken,
        addTokenToURL,
    ...
    function getToken() {
        ...
        return (((_c = (_b = (_a = directusApi.defaults) === null || _a === void 0 ? void 0 : _a.headers) === null || _b === void 0 ? void 0 : _b['Authorization']) === null || _c === void 0 ? void 0 : _c.split(' ')[1]) ||
            ((_g = (_f = (_e = (_d = directusApi.defaults) === null || _d === void 0 ? void 0 : _d.headers) === null || _e === void 0 ? void 0 : _e.common) === null || _f === void 0 ? void 0 : _f['Authorization']) === null || _g === void 0 ? void 0 : _g.toString().split(' ')[1]) ||
            null);
    }
...
function usePreset(collection, bookmark = ref(null), temporary = false) {
    const { usePresetsStore, useUserStore } = useStores$1();
    ...
    const autoSave = lodash.exports.debounce(async () => {
        savePreset();
    }, 450);
    /**
    ...
    function handleChanges() {
        if (bookmarkExists.value) {
            ...
            bookmarkSaved.value = lodash.exports.isEqual(localPreset.value, bookmarkInStore);
        }
        else {
            autoSave();
        }
    }
    function updatePreset(preset, immediate) {
        localPreset.value = lodash.exports.assign({}, localPreset.value, preset);
    ...
    const layoutOptions = computed({
        get() {
            var _a;
            return ((_a = localPreset.value.layout_options) === null || _a === void 0 ? void 0 : _a[layout.value]) || null;
        },
        set(options) {
            const { layout_options } = localPreset.value;
            updatePreset({ layout_options: lodash.exports.assign({}, layout_options, { [layout.value]: options }) });
        },
    });
    const layoutQuery = computed({
        get() {
            var _a;
            return ((_a = localPreset.value.layout_query) === null || _a === void 0 ? void 0 : _a[layout.value]) || null;
        },
        set(query) {
            const { layout_query } = localPreset.value;
            updatePreset({ layout_query: lodash.exports.assign({}, layout_query, { [layout.value]: query }) });
        },
    });
    ...
    const filter = computed({
        get: () => { var _a; return (_a = localPreset.value.filter) !== null && _a !== void 0 ? _a : null; },
    ...
    const bookmarkTitle = computed({
        get: () => { var _a; return ((_a = localPreset.value) === null || _a === void 0 ? void 0 : _a.bookmark) || null; },
    ...
    function clearLocalSave() {
        ...
        if (defaultPreset)
            localPreset.value = { ...defaultPreset };
        bookmarkSaved.value = true;
    }
    ...
    function initLocalPreset() {
        ...
        if (bookmark.value === null) {
            lodash.exports.assign(preset, presetsStore.getPresetForCollection(collection.value));
        }
        else if (bookmarkExists.value) {
            lodash.exports.assign(preset, presetsStore.getBookmark(Number(bookmark.value)));
        }
...
var BaseJoi = joiBrowser_min.exports;
...
var script$4 = defineComponent({
    ...
    setup(props, { emit }) {
        ...
        const mainElement = inject('main-element');
        ...
        watch([mainElementWidth, filterElementWidth], () => {
            var _a;
            ...
            const headerElement = (_a = mainElement === null || mainElement === void 0 ? void 0 : mainElement.value) === null || _a === void 0 ? void 0 : _a.firstElementChild;
            if (!headerElement)
                return;
            const maxWidth = searchElement.getBoundingClientRect().right -
        ...
        function onClickOutside(e) {
            ...
            if (path.some((el) => { var _a; return (_a = el === null || el === void 0 ? void 0 : el.classList) === null || _a === void 0 ? void 0 : _a.contains('v-menu-content'); }))
                return false;
            return true;
        }
        function disable() {
        ...
        function emitValue() {
            ...
            if (!input.value)
                return;
            const value = (_a = input.value) === null || _a === void 0 ? void 0 : _a.value;
            emit('update:modelValue', value);
        }
    },
});

const _hoisted_1$3 = ["value", "placeholder"];
const _hoisted_2$3 = {
  ref: "filterElement",
  class: "filter"
};
...
function render$4(_ctx, _cache, $props, $setup, $data, $options) {
  ...
  return (openBlock(), createBlock(_component_v_badge, {
    ...
    right: "",
    class: "search-badge",
    value: _ctx.activeFilterCount,
    disabled: !_ctx.activeFilterCount || _ctx.filterActive
  }, {
    default: withCtx(() => [
      withDirectives((openBlock(), createElementBlock("div", {
        class: normalizeClass(["search-input", { active: _ctx.active, 'filter-active': _ctx.filterActive, 'has-content': !!_ctx.modelValue, 'filter-border': _ctx.filterBorder }]),
        onClick: _cache[7] || (_cache[7] = $event => (_ctx.active = true))
      }, [
        withDirectives(createVNode(_component_v_icon, {
          name: "search",
          class: "icon-search",
          clickable: !_ctx.active
        }, null, 8 /* PROPS */, ["clickable"]), [
          [
            _directive_tooltip,
            _ctx.active ? null : _ctx.t('search'),
            void 0,
            { bottom: true }
          ]
        ]),
        createElementVNode("input", {
          ...
          placeholder: _ctx.t('search_items'),
          onInput: _cache[0] || (_cache[0] = (...args) => (_ctx.emitValue && _ctx.emitValue(...args))),
          onPaste: _cache[1] || (_cache[1] = (...args) => (_ctx.emitValue && _ctx.emitValue(...args)))
        ...
        (_ctx.modelValue)
          ? (openBlock(), createBlock(_component_v_icon, {
              ...
              class: "icon-empty",
              name: "close",
              onClick: _cache[2] || (_cache[2] = withModifiers($event => (_ctx.$emit('update:modelValue', null)), ["stop"]))
            }))
        ...
        withDirectives(createVNode(_component_v_icon, {
          ...
          class: "icon-filter",
          name: "filter_list",
          onClick: _cache[3] || (_cache[3] = $event => (_ctx.filterActive = !_ctx.filterActive))
        }, null, 512 /* NEED_PATCH */), [
          [
            _directive_tooltip,
            _ctx.t('filter'),
        ...
        createVNode(_component_transition_expand, {
          onBeforeEnter: _cache[5] || (_cache[5] = $event => (_ctx.filterBorder = true)),
          onAfterLeave: _cache[6] || (_cache[6] = $event => (_ctx.filterBorder = false))
        }, {
          default: withCtx(() => [
            withDirectives(createElementVNode("div", _hoisted_2$3, [
              createVNode(_component_interface_system_filter, {
                class: "filter-input",
                inline: "",
                value: _ctx.filter,
                "collection-name": _ctx.collection,
                onInput: _cache[4] || (_cache[4] = $event => (_ctx.$emit('update:filter', $event)))
              }, null, 8 /* PROPS */, ["value", "collection-name"])
            ], 512 /* NEED_PATCH */), [
              [vShow, _ctx.filterActive]
            ])
          ]),
          _: 1 /* STABLE */
        })
        ...
        [_directive_click_outside, {
				handler: _ctx.disable,
				middleware: _ctx.onClickOutside,
			}]
      ])
    ]),
    _: 1 /* STABLE */
...
function render$3(_ctx, _cache, $props, $setup, $data, $options) {
  ...
  return (openBlock(), createBlock(resolveDynamicComponent(_ctx.layoutWrapper), {
    selection: _ctx.layoutSelection,
    "onUpdate:selection": _cache[3] || (_cache[3] = $event => ((_ctx.layoutSelection) = $event)),
    "layout-options": _ctx.localOptions,
    "onUpdate:layout-options": _cache[4] || (_cache[4] = $event => ((_ctx.localOptions) = $event)),
    "layout-query": _ctx.localQuery,
    "onUpdate:layout-query": _cache[5] || (_cache[5] = $event => ((_ctx.localQuery) = $event)),
    filter: _ctx.layoutFilter,
    search: _ctx.search,
    collection: _ctx.collection,
    "select-mode": "",
    "show-select": _ctx.multiple ? 'multiple' : 'one'
  }, {
    default: withCtx(({ layoutState }) => [
      createVNode(_component_v_drawer, {
        modelValue: _ctx.internalActive,
        "onUpdate:modelValue": _cache[2] || (_cache[2] = $event => ((_ctx.internalActive) = $event)),
        title: _ctx.t('select_item'),
        onCancel: _ctx.cancel
      }, {
        subtitle: withCtx(() => [
          createVNode(_component_v_breadcrumb, {
            items: [{ name: _ctx.collectionInfo.name, disabled: true }]
          }, null, 8 /* PROPS */, ["items"])
        ]),
          ...
          createVNode(_component_v_button, {
            ...
            default: withCtx(() => [
              createVNode(_component_v_icon, {
                name: _ctx.collectionInfo.icon,
                color: _ctx.collectionInfo.color
              }, null, 8 /* PROPS */, ["name", "color"])
            ]),
            _: 1 /* STABLE */
          })
        ]),
        "actions:prepend": withCtx(() => [
          (openBlock(), createBlock(resolveDynamicComponent(`layout-actions-${_ctx.localLayout}`), normalizeProps(guardReactiveProps(layoutState)), null, 16 /* FULL_PROPS */))
        ]),
        actions: withCtx(() => [
          createVNode(_component_search_input, {
            modelValue: _ctx.search,
            "onUpdate:modelValue": _cache[0] || (_cache[0] = $event => ((_ctx.search) = $event)),
            filter: _ctx.presetFilter,
            "onUpdate:filter": _cache[1] || (_cache[1] = $event => ((_ctx.presetFilter) = $event)),
            collection: _ctx.collection
          }, null, 8 /* PROPS */, ["modelValue", "filter", "collection"]),
          withDirectives((openBlock(), createBlock(_component_v_button, {
            ...
            default: withCtx(() => [
              createVNode(_component_v_icon, { name: "check" })
            ]),
            _: 1 /* STABLE */
          }, 8 /* PROPS */, ["onClick"])), [
            [
        ...
        default: withCtx(() => [
          (openBlock(), createBlock(resolveDynamicComponent(`layout-${_ctx.localLayout}`), mergeProps({ class: "layout" }, layoutState), {
            "no-results": withCtx(() => [
              createVNode(_component_v_info, {
                title: _ctx.t('item_count', 0),
                icon: _ctx.collectionInfo.icon,
                center: ""
    ...
    ? (openBlock(), createElementBlock("div", _hoisted_1$2, [
        (_ctx.type === 'image')
          ? (openBlock(), createElementBlock("div", {
              key: 0,
              class: normalizeClass(["image", { svg: _ctx.isSVG, 'max-size': _ctx.inModal === false }]),
              ...
              createElementVNode("img", {
                ...
                height: _ctx.height,
                alt: _ctx.title,
                onError: _cache[0] || (_cache[0] = $event => (_ctx.imgError = true))
...
function generateJoi(filter, options) {
    filter = filter || {};
    options = lodash.exports.merge({}, defaults, options);
    const schema = {};
    for (const [key, value] of Object.entries(filter)) {
        ...
        if (isField) {
            ...
            if (operator === '_starts_with') {
                schema[key] = Joi.string().pattern(new RegExp(`^${lodash.exports.escapeRegExp(Object.values(value)[0])}.*`), {
                    name: 'starts_with',
                });
            }
...
function isAllowed(collection, action, value, strict = false) {
    const { usePermissionsStore, useUserStore } = useStores$1();
    const permissionsStore = usePermissionsStore();
    const userStore = useUserStore();
    ...
    const permissionInfo = permissions.find((permission) => permission.action === action && permission.collection === collection);
    if (!permissionInfo)
        return false;
    if (!permissionInfo.fields && action !== 'share')
        return false;
    if (strict && action !== 'share' && permissionInfo.fields.includes('*') === false && value) {
        ...
        if (attemptedFields.every((field) => allowedFields.includes(field)) === false)
            return false;
    }
...
function usePermissions(collection, item, isNew) {
    const { useUserStore, usePermissionsStore } = useStores$1();
    const userStore = useUserStore();
    const permissionsStore = usePermissionsStore();
    const { info: collectionInfo, fields: rawFields } = useCollection$1(collection);
    ...
    const archiveAllowed = computed(() => {
        var _a, _b;
        if (!((_b = (_a = collectionInfo.value) === null || _a === void 0 ? void 0 : _a.meta) === null || _b === void 0 ? void 0 : _b.archive_field))
            return false;
    ...
    const fields = computed(() => {
        var _a, _b, _c;
        let fields = lodash.exports.cloneDeep(rawFields.value);
        if (((_b = (_a = userStore.currentUser) === null || _a === void 0 ? void 0 : _a.role) === null || _b === void 0 ? void 0 : _b.admin_access) === true)
            return fields;
        const permissions = permissionsStore.getPermissionsForUser(collection.value, isNew.value ? 'create' : 'update');
        if (!permissions)
            return fields;
        if (((_c = permissions.fields) === null || _c === void 0 ? void 0 : _c.includes('*')) === false) {
            fields = fields.map((field) => {
                var _a;
                if (((_a = permissions.fields) === null || _a === void 0 ? void 0 : _a.includes(field.field)) === false) {
                    field.meta = {
                        ...(field.meta || {}),
                        readonly: true,
                    };
    ...
    const revisionsAllowed = computed(() => {
        ...
        if (((_b = (_a = userStore.currentUser) === null || _a === void 0 ? void 0 : _a.role) === null || _b === void 0 ? void 0 : _b.admin_access) === true)
            return true;
        return !!permissionsStore.permissions.find((permission) => permission.collection === 'directus_revisions' && permission.action === 'read');
    });
    return {
        createAllowed,
        deleteAllowed,
...
function useTemplateData(collection, primaryKey) {
    const templateData = ref();
    const loading = ref(false);
    const error = ref(null);
    const fields = computed(() => {
        var _a, _b;
        if (!((_b = (_a = collection.value) === null || _a === void 0 ? void 0 : _a.meta) === null || _b === void 0 ? void 0 : _b.display_template))
            return null;
    ...
    async function fetchTemplateValues() {
        if (!primaryKey.value || primaryKey.value === '+' || fields.value === null || !collection.value)
            return;
        loading.value = true;
...
var script$1 = defineComponent({
    ...
    setup(props, { emit }) {
        const { t, te } = useI18n();
        const { useFieldsStore, useRelationsStore } = useStores$1();
        const fieldsStore = useFieldsStore();
        const relationsStore = useRelationsStore();
        const { internalActive } = useActiveState();
        ...
        const { save, cancel } = useActions();
        const { collection } = toRefs(props);
        const { info: collectionInfo } = useCollection$1(collection);
        const title = computed(() => {
            const collection = (junctionRelatedCollectionInfo === null || junctionRelatedCollectionInfo === void 0 ? void 0 : junctionRelatedCollectionInfo.value) || collectionInfo.value;
        ...
        const showDivider = computed(() => {
            return (fieldsStore.getFieldsForCollection(props.collection).filter((field) => { var _a; return ((_a = field.meta) === null || _a === void 0 ? void 0 : _a.hidden) !== true; })
                .length > 0);
        });
        const { fields: junctionRelatedCollectionFields } = usePermissions(junctionRelatedCollection, computed(() => item.value && item.value[props.junctionField]), computed(() => props.primaryKey === '+'));
        const { fields: fieldsWithPermissions } = usePermissions(collection, item, computed(() => props.primaryKey === '+'));
        const fields = computed(() => {
        ...
        const templatePrimaryKey = computed(() => junctionFieldInfo.value ? String(props.relatedPrimaryKey) : String(props.primaryKey));
        ...
        const template = computed(() => {
            var _a, _b, _c, _d;
            return ((_b = (_a = junctionRelatedCollectionInfo.value) === null || _a === void 0 ? void 0 : _a.meta) === null || _b === void 0 ? void 0 : _b.display_template) ||
                ((_d = (_c = collectionInfo.value) === null || _c === void 0 ? void 0 : _c.meta) === null || _d === void 0 ? void 0 : _d.display_template) ||
                null;
        });
        ...
        function useFile() {
            ...
            const file = computed(() => {
                ...
                if (isDirectusFiles.value === false || !item.value)
                    return null;
                const fileData = (_a = item.value) === null || _a === void 0 ? void 0 : _a[props.junctionField];
                if (!fileData)
                    return null;
                const src = addTokenToURL(getRootPath() + `assets/${fileData.id}?key=system-large-contain`, getToken());
                return { ...fileData, src };
            });
        ...
        function useJunction() {
            ...
            const junctionRelatedCollection = computed(() => {
                ...
                if (relationForField.related_collection)
                    return relationForField.related_collection;
                if ((_a = relationForField.meta) === null || _a === void 0 ? void 0 : _a.one_collection_field)
                    return (props.edits[relationForField.meta.one_collection_field] ||
                        ((_b = item.value) === null || _b === void 0 ? void 0 : _b[relationForField.meta.one_collection_field]));
                return null;
            });
            const { info: junctionRelatedCollectionInfo } = useCollection$1(junctionRelatedCollection);
...
function render$1(_ctx, _cache, $props, $setup, $data, $options) {
  ...
  const _component_v_icon = resolveComponent("v-icon");
  const _component_v_button = resolveComponent("v-button");
  const _component_file_preview = resolveComponent("file-preview");
  ...
  return (openBlock(), createBlock(_component_v_drawer, {
    modelValue: _ctx.internalActive,
    "onUpdate:modelValue": _cache[1] || (_cache[1] = $event => ((_ctx.internalActive) = $event)),
    title: _ctx.title,
    persistent: "",
    onCancel: _ctx.cancel
  }, createSlots({
      ...
      createElementVNode("div", _hoisted_2$1, [
        (_ctx.junctionField)
          ? (openBlock(), createElementBlock(Fragment, { key: 0 }, [
              (_ctx.file)
                ? (openBlock(), createBlock(_component_file_preview, {
                    key: 0,
                    src: _ctx.file.src,
                    mime: _ctx.file.type,
                    width: _ctx.file.width,
                    height: _ctx.file.height,
                    title: _ctx.file.title,
                    "in-modal": true
                  }, null, 8 /* PROPS */, ["src", "mime", "width", "height", "title"]))
                : createCommentVNode("v-if", true),
              createVNode(_component_v_form, {
                disabled: _ctx.disabled,
                loading: _ctx.loading,
                "initial-values": _ctx.item && _ctx.item[_ctx.junctionField],
                "primary-key": _ctx.relatedPrimaryKey,
                "model-value": _ctx.internalEdits[_ctx.junctionField],
                fields: _ctx.junctionRelatedCollectionFields,
                autofocus: "",
                "onUpdate:modelValue": _ctx.setJunctionEdits
              }, null, 8 /* PROPS */, ["disabled", "loading", "initial-values", "primary-key", "model-value", "fields", "onUpdate:modelValue"]),
              (_ctx.showDivider)
                ? (openBlock(), createBlock(_component_v_divider, { key: 1 }))
                : createCommentVNode("v-if", true)
            ], 64 /* STABLE_FRAGMENT */))
          : createCommentVNode("v-if", true),
        createVNode(_component_v_form, {
          modelValue: _ctx.internalEdits,
          "onUpdate:modelValue": _cache[0] || (_cache[0] = $event => ((_ctx.internalEdits) = $event)),
          disabled: _ctx.disabled,
          loading: _ctx.loading,
          "initial-values": _ctx.item,
          "primary-key": _ctx.primaryKey,
          fields: _ctx.fields
        }, null, 8 /* PROPS */, ["modelValue", "disabled", "loading", "initial-values", "primary-key", "fields"])
      ])
    ]),
    _: 2 /* DYNAMIC */
  }, [
    (_ctx.template !== null && _ctx.templateData && _ctx.primaryKey !== '+')
      ? {
          name: "title",
          fn: withCtx(() => [
            (_ctx.loading || _ctx.templateDataLoading)
              ? (openBlock(), createBlock(_component_v_skeleton_loader, {
                  key: 0,
                  class: "title-loader",
                  type: "text"
                }))
              : (openBlock(), createElementBlock("h1", _hoisted_1$1, [
                  createVNode(_component_render_template, {
                    collection: _ctx.templateCollection.collection,
                    item: _ctx.templateData,
                    template: _ctx.template
                  }, null, 8 /* PROPS */, ["collection", "item", "template"])
                ]))
          ])
        }
      : undefined
...
var script = defineComponent({
    ...
    setup(props, { emit }) {
        ...
        const templateWithDefaults = computed(() => {
            ...
            if ((_a = junctionCollection.value.meta) === null || _a === void 0 ? void 0 : _a.display_template)
                return junctionCollection.value.meta.display_template;
            let relatedDisplayTemplate = (_b = relationCollection.value.meta) === null || _b === void 0 ? void 0 : _b.display_template;
        ...
        const fields = computed(() => adjustFieldsForDisplays(getFieldsFromTemplate$1(templateWithDefaults.value), junctionCollection.value.collection));
        const { deleteItem, getUpdatedItems, getNewItems, getPrimaryKeys, getNewSelectedItems } = useActions(value, relationInfo, emitter);
        const { tableHeaders, items, initialItems, loading } = usePreview(value, fields, relationInfo, getNewSelectedItems, getUpdatedItems, getNewItems, getPrimaryKeys);
        const { currentlyEditing, editItem, editsAtStart, stageEdits, cancelEdit, relatedPrimaryKey, editModalActive } = useEdit(value, relationInfo, emitter);
        const { stageSelection, selectModalActive, selectedPrimaryKeys } = useSelection(items, initialItems, relationInfo, emitter);
        const { sort, sortItems, sortedItems } = useSort(relationInfo, fields, items, emitter);
        const { createAllowed, selectAllowed } = usePermissions$1(junctionCollection, relationCollection);
        const { showUpload, onUpload } = useUpload();
        const downloadUrl = computed(() => {
            if (relatedPrimaryKey.value === null || relationCollection.value.collection !== 'directus_files')
                return;
            const { addTokenToURL, getToken } = useDirectusToken(useApi());
...
const _withScopeId = n => (pushScopeId("data-v-0c1ce2ee"),n=n(),popScopeId(),n);
const _hoisted_1 = {
  key: 1,
  class: "many-to-many"
};
...
function render(_ctx, _cache, $props, $setup, $data, $options) {
  ...
  const _component_v_upload = resolveComponent("v-upload");
  const _component_v_card_text = resolveComponent("v-card-text");
  const _component_v_card_actions = resolveComponent("v-card-actions");
  const _component_v_card = resolveComponent("v-card");
  const _component_v_dialog = resolveComponent("v-dialog");

  return (!_ctx.junction || !_ctx.relation)
    ? (openBlock(), createBlock(_component_v_notice, {
        key: 0,
        type: "warning"
      }, {
        default: withCtx(() => [
          createTextVNode(toDisplayString(_ctx.t('relationship_not_setup')), 1 /* TEXT */)
        ]),
        _: 1 /* STABLE */
      }))
    : (openBlock(), createElementBlock("div", _hoisted_1, [
        (_ctx.loading)
          ? (openBlock(true), createElementBlock(Fragment, { key: 0 }, renderList((_ctx.value || []).length || 3, (n) => {
              return (openBlock(), createBlock(_component_v_skeleton_loader, {
                key: n,
                type: (_ctx.value || []).length > 4 ? 'block-list-item-dense' : 'block-list-item'
              }, null, 8 /* PROPS */, ["type"]))
            }), 128 /* KEYED_FRAGMENT */))
          : (_ctx.sortedItems.length === 0)
            ? (openBlock(), createBlock(_component_v_notice, { key: 1 }, {
                default: withCtx(() => [
                  createTextVNode(toDisplayString(_ctx.t('no_items')), 1 /* TEXT */)
                ]),
                _: 1 /* STABLE */
              }))
            : (openBlock(), createBlock(_component_v_list, { key: 2 }, {
                default: withCtx(() => [
                  createVNode(_component_draggable, {
                    "force-fallback": true,
                    "model-value": _ctx.sortedItems,
                    "item-key": "id",
                    handle: ".drag-handle",
                    disabled: !_ctx.junction.meta.sort_field,
                    "onUpdate:modelValue": _cache[1] || (_cache[1] = $event => (_ctx.sortItems($event)))
                  }, {
                    item: withCtx(({ element }) => [
                      createVNode(_component_v_list_item, {
                        dense: _ctx.sortedItems.length > 4,
                        block: "",
                        clickable: "",
                        onClick: $event => (_ctx.editItem(element))
                      }, {
                        default: withCtx(() => [
                          (_ctx.junction.meta.sort_field)
                            ? (openBlock(), createBlock(_component_v_icon, {
                                key: 0,
                                name: "drag_handle",
                                class: "drag-handle",
                            ...
                            : createCommentVNode("v-if", true),
                          createVNode(_component_render_template, {
                            collection: _ctx.junctionCollection.collection,
                            item: element,
                            template: _ctx.templateWithDefaults
                          }, null, 8 /* PROPS */, ["collection", "item", "template"]),
                          _hoisted_2,
                          (!_ctx.disabled)
                            ? (openBlock(), createBlock(_component_v_icon, {
                                key: 1,
                                name: "close",
                                onClick: withModifiers($event => (_ctx.deleteItem(element)), ["stop"])
                              }, null, 8 /* PROPS */, ["onClick"]))
                            : createCommentVNode("v-if", true)
                        ]),
                        _: 2 /* DYNAMIC */
                      }, 1032 /* PROPS, DYNAMIC_SLOTS */, ["dense", "onClick"])
                    ]),
                    _: 1 /* STABLE */
                  }, 8 /* PROPS */, ["model-value", "disabled"])
                ]),
                _: 1 /* STABLE */
              })),
        (!_ctx.disabled)
          ? (openBlock(), createElementBlock("div", _hoisted_3, [
              (_ctx.enableCreate && _ctx.createAllowed)
                ? (openBlock(), createBlock(_component_v_button, {
                    key: 0,
                    onClick: _cache[2] || (_cache[2] = $event => (_ctx.showUpload = true))
                  }, {
                    default: withCtx(() => [
                      createTextVNode(toDisplayString(_ctx.t('upload_file')), 1 /* TEXT */)
                    ]),
                    _: 1 /* STABLE */
                  }))
                : createCommentVNode("v-if", true),
              (_ctx.enableSelect && _ctx.selectAllowed)
                ? (openBlock(), createBlock(_component_v_button, {
                    key: 1,
                    onClick: _cache[3] || (_cache[3] = $event => (_ctx.selectModalActive = true))
                  }, {
                    default: withCtx(() => [
                      createTextVNode(toDisplayString(_ctx.t('add_existing')), 1 /* TEXT */)
                    ]),
                    _: 1 /* STABLE */
                  }))
                : createCommentVNode("v-if", true)
        ...
        (!_ctx.disabled)
          ? (openBlock(), createBlock(_component_drawer_item, {
              key: 4,
              active: _ctx.editModalActive,
              collection: _ctx.relationInfo.junctionCollection,
              "primary-key": _ctx.currentlyEditing || '+',
              "related-primary-key": _ctx.relatedPrimaryKey || '+',
              "junction-field": _ctx.relationInfo.junctionField,
              edits: _ctx.editsAtStart,
              "circular-field": _ctx.junction.field,
              onInput: _ctx.stageEdits,
              "onUpdate:active": _ctx.cancelEdit
            }, {
              actions: withCtx(() => [
                (_ctx.currentlyEditing !== '+' && _ctx.relationCollection.collection === 'directus_files')
                  ? (openBlock(), createBlock(_component_v_button, {
                      key: 0,
                      secondary: "",
                      rounded: "",
              ...
              _: 1 /* STABLE */
            }, 8 /* PROPS */, ["active", "collection", "primary-key", "related-primary-key", "junction-field", "edits", "circular-field", "onInput", "onUpdate:active"]))
          : createCommentVNode("v-if", true),
        (!_ctx.disabled)
          ? (openBlock(), createBlock(_component_drawer_collection, {
              key: 5,
              active: _ctx.selectModalActive,
        ...
        (!_ctx.disabled)
          ? (openBlock(), createBlock(_component_v_dialog, {
              key: 6,
              modelValue: _ctx.showUpload,
              "onUpdate:modelValue": _cache[6] || (_cache[6] = $event => ((_ctx.showUpload) = $event))
            }, {
              default: withCtx(() => [
                createVNode(_component_v_card, null, {
                  default: withCtx(() => [
                    ...
                    createVNode(_component_v_card_text, null, {
                      default: withCtx(() => [
                        createVNode(_component_v_upload, {
                          multiple: "",
                          "from-url": "",
                          folder: _ctx.folder,
                          onInput: _ctx.onUpload
                        }, null, 8 /* PROPS */, ["folder", "onInput"])
                      ]),
                      _: 1 /* STABLE */
                    }),
                    createVNode(_component_v_card_actions, null, {
                      default: withCtx(() => [
                        createVNode(_component_v_button, {
                          onClick: _cache[5] || (_cache[5] = $event => (_ctx.showUpload = false))
                        }, {
                          default: withCtx(() => [
                            createTextVNode(toDisplayString(_ctx.t('done')), 1 /* TEXT */)
                          ]),
                          _: 1 /* STABLE */
                        })
                      ]),
...
script.__scopeId = "data-v-0c1ce2ee";
script.__file = "src/files.vue";
        ...
        return [
            ...
            },
            {
                ...
                meta: {
                    ...
                    options: {
                        collectionName: (_a = relations.o2m) === null || _a === void 0 ? void 0 : _a.collection,
                    },
                },
            },
            {
